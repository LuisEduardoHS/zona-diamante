export function renderBottomNav() {
    // Detección robusta de la página actual usando el nombre del archivo
    const fullPath = window.location.pathname;
    const hrefFull = window.location.href;

    // Extraer el nombre del archivo desde la ruta
    const segments = fullPath.split('/').filter(Boolean);
    const currentFile = segments.length > 0 ? segments[segments.length - 1].toLowerCase() : '';

    // Detectar si estamos en una subcarpeta (/pages/) para ajustar rutas relativas
    const inSubfolder = fullPath.toLowerCase().includes('/pages/');
    const base = inSubfolder ? '../' : '';

    const isInicio = currentFile === 'index.html' || currentFile === '' || fullPath === '/';
    const isTrivia = currentFile === 'trivia.html';
    const isInfo = currentFile === 'informacion.html' || hrefFull.toLowerCase().includes('informacion.html');
    ...
    const navHTML = `
        <nav class="fixed bottom-0 left-0 w-full bg-[#c8c8c8] h-20 rounded-t-3xl z-50">
            <div class="flex justify-between items-center h-full px-2">
                ...
                <!-- 1. INICIO -->
                <a href="${base}index.html" class="relative w-1/5 h-full flex flex-col items-center justify-center transition-colors cursor-pointer no-underline ${isInicio ? 'text-[#2B2B31]' : 'text-slate-400 hover:text-[#2B2B31]'}">
                ...
                <!-- 2. TRIVIA -->
                <a href="${base}trivia.html" class="relative w-1/5 h-full flex flex-col items-center justify-center transition-colors cursor-pointer no-underline ${isTrivia ? 'text-[#2B2B31]' : 'text-slate-400 hover:text-[#2B2B31]'}">
                ...
                <div class="w-1/5 h-full relative">
                    <a href="${base}camara.html" class="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#f27b21] w-[72px] h-[72px] rounded-[1.25rem] flex items-center justify-center shadow-lg">
                ...
                <!-- 4. INFO -->
                <a href="${base}Informacion.html" class="relative w-1/5 h-full flex flex-col items-center justify-center transition-colors cursor-pointer no-underline ${isInfo ? 'text-[#2B2B31]' : 'text-slate-400 hover:text-[#2B2B31]'}">
                ...
                <!-- 5. JUEGO -->
                <a href="${base}juego.html" class="relative w-1/5 h-full flex flex-col items-center justify-center transition-colors cursor-pointer no-underline ${isJuego ? 'text-[#2B2B31]' : 'text-slate-400 hover:text-[#2B2B31]'}">
